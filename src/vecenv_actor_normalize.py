...
class ActorNeuralNetwork(nn.Module):
    ...
    def update_from_moments(self, batch_mean: Union[np.ndarray, Dict], batch_var: Union[np.ndarray, Dict], batch_count: Union[int, float, Dict]) -> None:
        if self.obs_keys is not None:
            for k in self.obs_keys:
    ...
    def forward(self, state) -> torch.Tensor:
        if (isinstance(state, dict)):
            if isinstance(state["observation"], torch.Tensor):
                ...
                _state = np.array([np.concatenate([o, d]) for o,d in zip(obs,des)])
                # _state = np.concatenate([obs, des], axis=1)
            else:
                _state = np.concatenate([state['observation'], state['desired_goal']])
            # _state = []
