...
class HerReplayBuffer(DictReplayBuffer):
    ...
    def _sample_online(self, batch_size: int, env: Optional[VecNormalize] = None) -> DictReplayBufferSamples:
        ...
        # real_batch_inds, virtual_batch_inds = np.split(batch_inds, [nb_virtual])
        # real_env_indices, virtual_env_indices = np.split(env_indices, [nb_virtual])
    ...
    def _get_virtual_samples(
        ...
        # Compute new reward
        rewards = self.single_env.compute_reward(obs["desired_goal"],next_obs["achieved_goal"],{})
        ...
        # rewards = rewards.squeeze(1).astype(np.float32)  # env_method returns a list containing one element
